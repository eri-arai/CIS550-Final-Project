...
export default class Recommendations extends React.Component {
	...
	submitMovie() {
		...
		fetch("http://localhost:8081/recommendations/" + this.state.movieName,
		{
			...
			console.log(recommendationsList); //displays your JSON object in the console
			let recommendationsDivs = recommendationsList.map((recommendation, i) => 
				// <RecommendationsRow key={recommendation.id} movie={recommendation} />
				<div id="results" key={i} className="results-container">
					<RecommendationsRow title={recommendation.title} id={recommendation.id} rating={recommendation.rating} vote_count={recommendation.vote_count}/>
				</div>
				
			);

			this.setState({
				recMovies: recommendationsDivs
			});
		});
	}
